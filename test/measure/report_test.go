...
func checkCounts(t *testing.T, method engine.MethodReport, sent int) {
	...
	if method.Latencies+method.Unanswered != sent {
		t.Errorf("%d calls have a latency and %d never reached the target, for %d calls",
			method.Latencies, method.Unanswered, sent)
	}
}
...
func TestReport_FailuresOnAScheduleAreCountedExactly(t *testing.T) {
	...
	// A failure that came back from the target is an observation like any
	// other: dropping it would hide exactly the calls load testing is about.
	if method.Latencies != sent {
		t.Errorf("%d calls of %d carry a latency; every one was answered", method.Latencies, sent)
	}
	if method.Censored != 0 {
		t.Errorf("%d answered calls were recorded as cut short", method.Censored)
	}
