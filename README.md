...
Проект держится на трёх приоритетах, в этом порядке: **корректность измерений** — числа
соответствуют тому, что происходило на самом деле, в том числе когда сервис деградирует;
**удобство** — конфиг, ошибки и отчёт рассчитаны на человека; **скорость** — генератор не
